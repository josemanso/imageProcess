...
std::vector<std::string> load_class_list()
{
  ...
  std::ifstream ifs("/home/josemo/opencv/yolo/classes.txt");
  std::string line;
...
{
  auto result =cv::dnn::readNet("/home/josemo/opencv/yolo/yolov5s.onnx");
  //
...
cv::Mat format_yolo5(const cv::Mat &source){
  int col = source.cols;
  int row = source.rows;
     ...
     const std::vector<std::string> &className)
{
  ...
  auto input_image =format_yolo5(image);
...
int main(int, char **)
{
  ...
  cv::Mat frame;
  cv::VideoCapture capture("/home/josemo/opencv/input/video_1.mp4");
  if (!capture.isOpened())
  {
